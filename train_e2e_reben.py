...
def evaluate_student_test_set_reben(encoder_model, student_model, test_loader, args, run_name):
    """
    Evaluate a trained StudentHead (AS4) on the reBEN test split. test_loader yields
    raw images (not pre-baked embeddings), so the encoder forward pass runs here first —
    mirrors utils.misc.evaluate_student_test_set, adapted for the e2e pipeline.
    """
    encoder_model.eval()
    student_model.eval()
...
def evaluate_uncertainty_correlation_reben(encoder_model, decoder, student_model, test_loader, args, run_name):
    """
    Compares spatial layout of teacher (ensemble) vs student (Dirichlet) uncertainty
    maps for reBEN e2e — mirrors utils.misc.evaluate_uncertainty_correlation, adapted
    to run the encoder once per batch and share features between teacher and student
    rather than encoding twice.
    """
    encoder_model.eval()
    decoder.eval()
    student_model.eval()
    ...
    log_msg(f"REBEN TEACHER/STUDENT UNCERTAINTY CORRELATION ({patch_count} patches, {skipped_degenerate} degenerate skipped):")
