...
def train_e2e_reben_sar(args):
    run_name = f"{args.run_name}_{time.strftime('%Y%m%d_%H%M')}"
    log_msg(f"Run name: {run_name}")
    log_msg(f"Running reBEN SAR E2E training with inputs:\n {vars(args)}")

    # 1. Data — official splits from metadata.parquet, paired via s1_name
    ...
    for epoch in range(start_epoch, args.num_epochs):
        ...
        for batch_imgs, batch_masks in train_loader:
            ...
            with torch.cuda.amp.autocast():
                features = get_encoder_representation_partial(batch_imgs, encoder_model, waves=S1_WAVES)
        ...
        with torch.no_grad():
            for v_imgs, v_masks in val_loader:
                ...
                with torch.cuda.amp.autocast():
                    features = get_encoder_representation_partial(v_imgs, encoder_model, waves=S1_WAVES)
                    all_preds = decoder(features)
                    total_val_loss = 0
                    for head_idx in range(decoder.M):
...
if __name__ == "__main__":
    ...
    parser.add_argument("--s1_root",       type=str, required=True,
                        help="Path to BigEarthNet-S1/ directory")
    ...
    # Misc
    parser.add_argument("--run_name",    type=str, default="reben_sar_e2e")
